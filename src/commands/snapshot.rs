...
use crate::snapshot::FileSnapshotService;
...
pub async fn handle_snapshot_command(
    cmd: SnapshotCommand,
    snapshot_service: &(impl FileSnapshotService + Sync),
) -> Result<()> {
    match cmd.command {
        SnapshotSubcommand::Create { file } => {
            let snapshot = snapshot_service.create_snapshot(&file).await?;
            println!(
                "Created snapshot: {} at {}",
                snapshot.timestamp,
