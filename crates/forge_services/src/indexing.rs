...
impl<F> ForgeIndexingService<F> {
        ...
        F: ContextEngineRepository,
    {
        ...
        let workspace_files =
            forge_domain::CodeBase::new(user_id.clone(), *workspace_id, ());
        ...
        if !files_to_delete.is_empty() {
            ...
            let deletion =
                forge_domain::CodeBase::new(user_id.clone(), *workspace_id, files_to_delete);
            self.infra
...
> ContextEngineService for ForgeIndexingService<F>
{
    async fn sync_codebase(&self, path: PathBuf, batch_size: usize) -> Result<FileUploadResponse> {
        ...
        // TODO: Fix workspace indexing integration
        // let existing_workspace = self.infra.find_by_path(&canonical_path).await?;
        let existing_workspace: Option<forge_domain::Workspace> = None;

        let (workspace_id, is_new_workspace) = match existing_workspace {
            Some(_) => (WorkspaceId::generate(), false),
        ...
        for batch in files_to_upload.chunks(batch_size) {
            ...
            let upload =
                forge_domain::CodeBase::new(user_id.clone(), workspace_id, file_reads);
    ...
    async fn query_codebase(
        ...
        let _workspace: Option<forge_domain::Workspace> = None;
        ...
        let search_query = forge_domain::CodeBase::new(
            // TODO: Fix workspace fields
            // workspace.user_id.clone(),
            // workspace.workspace_id.clone(),
            forge_domain::UserId::from_string("temp")?,
        ...
        F: WorkspaceRepository + ContextEngineRepository + ProviderRepository,
    {
        ...
        let workspace: Option<forge_domain::Workspace> = None;

        if let Some(_workspace) = workspace {
            // Get detailed workspace info from server
            self.infra
                .as_ref()
                // TODO: Fix workspace access
                // .get_workspace(&workspace.workspace_id, &token)
                .get_workspace(&WorkspaceId::generate(), &token)
                .await
    ...
    async fn is_indexed(&self, path: &std::path::Path) -> Result<bool> {
        ...
        let _canonical_path = match path.canonicalize() {
            Ok(p) => p,
...
mod tests {
    ...
    impl WorkspaceRepository for MockInfra {
        fn create_or_update_workspace(&self, _: WorkspaceId, _: &Path) -> Result<Workspace> {
            Ok(self.workspace.clone().unwrap_or_else(|| workspace()))
        }

        fn get_workspace_by_id(&self, _: WorkspaceId) -> Result<Option<Workspace>> {
            Ok(self.workspace.clone())
        }

        fn update_last_accessed(&self, _: WorkspaceId) -> Result<()> {
            Ok(())
        }

        fn mark_inactive(&self, _: WorkspaceId) -> Result<()> {
            Ok(())
        }

        fn ensure_workspace_metadata(&self, workspace_id: WorkspaceId, folder_path: &Path) -> Result<Workspace> {
            if let Some(existing) = self.workspace.clone() {
                if existing.folder_path.to_string_lossy() == "unknown" || existing.folder_path.as_os_str().is_empty() {
                    let mut updated = existing;
                    updated.folder_path = folder_path.to_path_buf();
                    updated.last_accessed_at = Some(chrono::Utc::now());
    ...
    async fn test_get_workspace_info_returns_workspace() {
        ...
        mock.workspaces.lock().await.push(WorkspaceInfo {
            workspace_id: ws.workspace_id.clone(),
            working_dir: ws.folder_path.to_str().unwrap().into(),
            node_count: 5,
            relation_count: 10,
        ...
        let actual = service.get_workspace_info(ws.folder_path).await.unwrap();
    ...
    async fn test_get_workspace_info_error_when_not_authenticated() {
        ...
        let ws = mock.workspace.clone().unwrap();
        let service = ForgeIndexingService::new(Arc::new(mock));
        let actual = service.get_workspace_info(ws.folder_path).await;
