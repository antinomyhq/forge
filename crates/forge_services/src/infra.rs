...
/// Infrastructure trait for accessing environment configuration and system variables.
///
/// This trait provides access to the application environment which includes configuration
/// for both global and project-local agent directories. The Environment exposes:
/// - Global agent directory via `agent_path()` (typically ~/.forge/agents)
/// - Project-local agent directory via `agent_cwd_path()` (typically .forge/agents)
