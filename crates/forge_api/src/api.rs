...
pub trait API: Sync + Send {
    ...
    /// Provides models from all configured providers. Providers that successfully
    /// return models are included in the result. If every configured provider
    /// fails (e.g. due to an invalid API key), the first error is returned so
    /// the caller sees the real underlying cause rather than an empty list.
