...
impl ForgeAPI<ForgeServices<ForgeRepo<ForgeInfra>>, ForgeRepo<ForgeInfra>> {
    pub fn init(restricted: bool, cwd: PathBuf) -> Self {
        let infra = Arc::new(ForgeInfra::new(restricted, cwd.clone()));
        let repo = Arc::new(ForgeRepo::new(infra.clone()));
        let app = Arc::new(ForgeServices::new(repo.clone()));

        // Automatically update workspace metadata if needed
        let workspace_id = repo.get_environment().workspace_id();
        let _ = repo.ensure_workspace_metadata(workspace_id, &cwd);
