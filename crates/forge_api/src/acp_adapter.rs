...
//!
//! This module contains the internal ACP protocol adapter that translates
//! between ACP protocol types and Forge domain types. It is NOT exported
//! publicly - all ACP functionality is exposed through the API trait.
...
use crate::acp_conversion;
use crate::acp_error::{Error, Result};
...
impl<S: Services> AcpAdapter<S> {
    ...
    async fn build_session_mode_state(
        ...
        Ok(acp_conversion::build_session_mode_state(
            &agents,
            current_agent_id,
        ))
    }
...
impl<S: Services> acp::Agent for AcpAdapter<S> {
        ...
        for content_block in &arguments.prompt {
            match content_block {
                ...
                acp::ContentBlock::ResourceLink(resource_link) => {
                    ...
                    let path = acp_conversion::uri_to_path(&resource_link.uri);
                ...
                acp::ContentBlock::Resource(embedded_resource) => {
                    ...
                    match acp_conversion::acp_resource_to_attachment(embedded_resource) {
                                    ...
                                    response_result = stream.next() => {
                                        ...
                                        match response {
                                            ...
                                            forge_domain::ChatResponse::ToolCallStart(tool_call) => {
                                                ...
                                                let acp_tool_call = acp_conversion::map_tool_call_to_acp(&tool_call);
                                            ...
                                            forge_domain::ChatResponse::ToolCallEnd(tool_result) => {
                                                ...
                                                let content = acp_conversion::map_tool_output_to_content(&tool_result.output);
