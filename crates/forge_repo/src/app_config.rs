...
impl From<AppConfig> for forge_domain::ForgeConfig {
    fn from(value: AppConfig) -> Self {
        forge_domain::ForgeConfig {
...
impl From<forge_domain::ForgeConfig> for AppConfig {
    fn from(value: forge_domain::ForgeConfig) -> Self {
        AppConfig {
            key_info: value.key_info,
            provider: value.provider,
            model: value.model,
...
{
    async fn get_app_config(&self) -> anyhow::Result<forge_domain::ForgeConfig> {
        // Check cache first
    ...
    async fn set_app_config(&self, config: &forge_domain::ForgeConfig) -> anyhow::Result<()> {
        let (model, provider) = self.get_overrides();
...
mod tests {
    ...
    fn repository_with_config_fixture() -> (AppConfigRepositoryImpl<MockInfra>, TempDir) {
        ...
        let config = forge_domain::ForgeConfig::default();
    ...
    async fn test_get_app_config_exists() {
        let expected = forge_domain::ForgeConfig::default();
    ...
    async fn test_set_app_config() {
        let fixture = forge_domain::ForgeConfig::default();
        let (repo, _temp_dir) = repository_fixture();
    ...
    async fn test_cache_behavior() {
        ...
        let new_config = forge_domain::ForgeConfig::default();
    ...
    async fn test_read_handles_custom_provider() {
        ...
        let expected = forge_domain::ForgeConfig {
    ...
    async fn test_read_returns_default_if_not_exists() {
        ...
        assert_eq!(config, forge_domain::ForgeConfig::default());
    }
    ...
    async fn test_override_model() {
        ...
        let mut config = forge_domain::ForgeConfig::default();
        config.model.insert(
    ...
    async fn test_override_provider() {
        ...
        let config = forge_domain::ForgeConfig { provider: Some(ProviderId::ANTHROPIC), ..Default::default() };
        let content = serde_json::to_string_pretty(&config).unwrap();
    ...
    async fn test_read_returns_default_on_unrepairable_json() {
        ...
        assert_eq!(actual, forge_domain::ForgeConfig::default());
    }
}
