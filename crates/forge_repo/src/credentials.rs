...
impl From<&WorkspaceAuth> for IndexingAuthModel {
...
mod tests {
    ...
    async fn test_store_and_retrieve_auth() {
        ...
        let expected = WorkspaceAuth::new(UserId::generate(), "test_token_123".to_string().into());
    ...
    async fn test_logout() {
        ...
        let auth = WorkspaceAuth::new(UserId::generate(), "test_token".to_string().into());
    ...
    async fn test_replace_existing_auth() {
        ...
        // Store first auth
        let auth1 = WorkspaceAuth::new(user_id.clone(), "token1".to_string().into());
        repo.set_auth(&auth1).await.unwrap();

        // Store second auth with same user_id (should replace)
        let auth2 = WorkspaceAuth::new(user_id, "token2".to_string().into());
