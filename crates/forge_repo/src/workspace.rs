...
use anyhow::Result;
use chrono::Utc;
use forge_domain::{WorkspaceId, Workspace as WorkspaceDomain, WorkspaceRepository};
...
impl WorkspaceRepository for WorkspaceRepositoryImpl {
    fn create_or_update_workspace(&self, workspace_id: WorkspaceId, folder_path: &Path) -> Result<WorkspaceDomain> {
...
mod tests {
    ...
    fn test_workspace_repository_create_and_get() -> anyhow::Result<()> {
        ...
        // Create workspace
        let workspace_result = workspace_repo.create_or_update_workspace(workspace_id, &folder_path)?;
