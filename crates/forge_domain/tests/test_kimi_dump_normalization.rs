...
async fn test_kimi_k2p5_turbo_dump_normalization() {
    // Load the dump file
    let dump_path =
        PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("../../kimi-k2p5-turbo-error-dump.json");
    ...
    let dump: ConversationDump =
        serde_json::from_str(&content).expect("Failed to parse conversation dump");
    ...
    assert!(
        patch_tool_calls_found >= 2,
        "Expected at least 2 patch tool calls, found {}",
        patch_tool_calls_found
    );

    // All patch tool calls should have properly normalized arguments
    assert!(
        errors.is_empty(),
        "Found {} patch tool calls with string arguments that weren't normalized:\n{}",
        errors.len(),
        errors.join("\n")
    );
}

/// Test that serializing the normalized context produces JSON objects for
/// arguments
#[tokio::test]
async fn test_kimi_k2p5_turbo_dump_serialization() {
    ...
    // Serialize to JSON
    let serialized =
        serde_json::to_string(&normalized).expect("Failed to serialize normalized context");
    ...
    assert!(
        errors.is_empty(),
        "Found {} tool calls with string arguments in serialized output:\n{}",
        errors.len(),
        errors.join("\n")
    );
}
