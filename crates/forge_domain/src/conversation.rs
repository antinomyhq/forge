...
impl Conversation {
    ...
    pub fn new(id: ConversationId, workflow: Workflow, additional_tools: Vec<ToolName>) -> Self {
        // FIXME: add base_agents
        Self::new_inner(id, workflow, Default::default(), additional_tools)
    }
    ...
    fn new_inner(
        ...
        let mut all_agents = Vec::new(); // No agents from workflow anymore
        all_agents.extend(base_agents);
        for mut agent in all_agents.into_iter() {
            ...
            if let Some(ref workflow_compact) = workflow.compact {
                if let Some(ref mut agent_compact) = agent.compact {
                    ...
                    let agent_compact_clone: Compact = agent_compact.clone();
                    merged_compact.merge(agent_compact_clone);
            ...
            if agent.id == AgentId::default() {
                ...
                if let Some(ref mut subscriptions) = agent.subscribe {
                    subscriptions.extend(commands.into_iter());
                } else {
