...
/// for refactoring tasks (e.g., renaming variables, updating function signatures).
/// For maximum efficiency, invoke multiple `patch` operations simultaneously
/// rather than sequentially. Fails if search pattern isn't found.
///
/// Usage Guidelines:
/// - When editing text from Read tool output, preserve the EXACT text character-by-character
