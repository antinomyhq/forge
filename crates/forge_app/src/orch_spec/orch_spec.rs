...
#[tokio::test]
async fn test_not_complete_when_no_stop_finish_reason() {
    // Test: is_complete = false when finish_reason is not Stop
    let mut ctx = TestContext::default().mock_assistant_responses(vec![
        ChatCompletionMessage::assistant("First message"),
        ChatCompletionMessage::assistant("Second message"),
        ChatCompletionMessage::assistant("Third message").finish_reason(FinishReason::Stop),
    ]);
    ...
    // Verify we have exactly 3 assistant messages (continued until Stop)
    let assistant_message_count = messages
        .iter()
        .filter(|message| message.has_role(Role::Assistant))
        .count();
    assert_eq!(
        assistant_message_count, 3,
        "Should have 3 assistant messages, confirming is_complete was false without Stop finish_reason"
    );
}

#[tokio::test]
async fn test_should_yield_when_complete() {
    // Test: should_yield = true when is_complete is true
    let mut ctx = TestContext::default().mock_assistant_responses(vec![
        ChatCompletionMessage::assistant(Content::full("All done"))
            .finish_reason(FinishReason::Stop),
    ]);

    ctx.run("Simple task").await.unwrap();

    // Verify TaskComplete is sent (which means should_yield was true)
    let has_task_complete = ctx
        .output
        .chat_responses
        .iter()
    ...
    assert!(
        has_task_complete,
        "Should yield and send TaskComplete when is_complete is true"
    );
}

#[tokio::test]
async fn test_should_yield_with_followup_tool() {
    // Test: should_yield = true when any tool call should yield (followup)
    let followup_call =
        ToolCallFull::new("followup").arguments(json!({"question": "Need more information?"}));
    let followup_result =
        ToolResult::new("followup").output(Ok(ToolOutput::text("Follow-up sent")));

    let mut ctx = TestContext::default()
        .mock_tool_call_responses(vec![(followup_call.clone(), followup_result)])
        .mock_assistant_responses(vec![
            ChatCompletionMessage::assistant("Asking for input")
                .tool_calls(vec![followup_call.into()]),
        ]);

    ctx.run("Ask for more info").await.unwrap();

    // Verify the orchestrator yielded after the followup tool
    // (no more assistant messages should be processed after followup)
    let messages = ctx.output.context_messages();
    let assistant_message_count = messages
        .iter()
        .filter(|message| message.has_role(Role::Assistant))
    ...
    assert_eq!(
        ...
        "Should yield after followup tool, not continuing conversation"
    );
}

#[tokio::test]
async fn test_should_yield_with_followup_even_when_not_complete() {
    // Test: should_yield = true when tool should yield, even if not complete
    let followup_call =
        ToolCallFull::new("followup").arguments(json!({"question": "More details?"}));
    let followup_result = ToolResult::new("followup").output(Ok(ToolOutput::text("Asked user")));

    let mut ctx = TestContext::default()
        .mock_tool_call_responses(vec![(followup_call.clone(), followup_result)])
        .mock_assistant_responses(vec![
            ChatCompletionMessage::assistant("I need clarification")
                .tool_calls(vec![followup_call.into()]), // No Stop finish_reason
        ]);

    ctx.run("Complex task").await.unwrap();

    // Verify it yielded without TaskComplete (since is_complete is false)
    let has_task_complete = ctx
        .output
        .chat_responses
        .iter()
    ...
    assert!(
        !has_task_complete,
        "Should NOT have TaskComplete since is_complete is false"
    );

    // But verify it did yield (only 1 assistant message)
    let messages = ctx.output.context_messages();
    let assistant_message_count = messages
        .iter()
        .filter(|message| message.has_role(Role::Assistant))
    ...
    assert_eq!(
        assistant_message_count, 1,
        "Should yield after followup even when not complete"
    );
}

#[tokio::test]
async fn test_should_not_yield_with_regular_tool_calls() {
    // Test: should_yield = false when is_complete is false and no yielding tools
    let tool_call = ToolCallFull::new("fs_read")
        .arguments(ToolCallArguments::from(json!({"path": "file.txt"})));
    let tool_result = ToolResult::new("fs_read").output(Ok(ToolOutput::text("content")));

    let mut ctx = TestContext::default()
        .mock_tool_call_responses(vec![(tool_call.clone(), tool_result)])
        .mock_assistant_responses(vec![
            ChatCompletionMessage::assistant("Reading").tool_calls(vec![tool_call.into()]),
            ChatCompletionMessage::assistant("Done").finish_reason(FinishReason::Stop),
        ]);

    ctx.run("Read file").await.unwrap();

    let messages = ctx.output.context_messages();

    // Verify conversation continued (did not yield after first tool call)
    let assistant_message_count = messages
        .iter()
        .filter(|message| message.has_role(Role::Assistant))
        .count();
    ...
    assert_eq!(
        assistant_message_count, 2,
        "Should not yield with regular tool calls, continuing until complete"
    );
}

#[tokio::test]
async fn test_mixed_regular_and_followup_tools_should_yield() {
    // Test: should_yield = true when any tool (even one) should yield
    let fs_tool_call = ToolCallFull::new("fs_read")
        .arguments(ToolCallArguments::from(json!({"path": "test.txt"})));
    let fs_tool_result = ToolResult::new("fs_read").output(Ok(ToolOutput::text("file data")));

    let followup_call =
        ToolCallFull::new("followup").arguments(json!({"question": "Clarification needed?"}));
    let followup_result = ToolResult::new("followup").output(Ok(ToolOutput::text("Question sent")));

    let mut ctx = TestContext::default()
        .mock_tool_call_responses(vec![
            (fs_tool_call.clone(), fs_tool_result),
            (followup_call.clone(), followup_result),
        ])
        .mock_assistant_responses(vec![
            ChatCompletionMessage::assistant("Reading and asking")
                .tool_calls(vec![fs_tool_call.into(), followup_call.into()]),
        ]);

    ctx.run("Read and ask").await.unwrap();

    // Verify it yielded (only 1 assistant message)
    let messages = ctx.output.context_messages();
    let assistant_message_count = messages
        .iter()
        .filter(|message| message.has_role(Role::Assistant))
    ...
    assert_eq!(
        assistant_message_count, 1,
        "Should yield when any tool call requires yielding"
    );
}
