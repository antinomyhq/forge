...
mod tests {
    ...
    struct MockAgentService(Handlebars<'static>);
    ...
    impl AgentService for MockAgentService {
        ...
        /// Execute a tool call
        async fn call(&self, _: &Agent, _: &mut ToolCallContext, _: ToolCallFull) -> ToolResult {
            unimplemented!()
        }
        ...
        async fn render(
            ...
            Ok(self.0.render_template(template, object)?)
        }

        /// Synchronize the on-going conversation
        async fn update(&self, _: Conversation) -> anyhow::Result<()> {
            unimplemented!()
        }
    }
    ...
    fn env() -> Environment {
        Environment {
            ...
            forge_api_url: Url::parse("http://localhost:3000").unwrap(),
    ...
    impl Default for Orchestrator<MockAgentService> {
        fn default() -> Self {
            let conversation =
                Conversation::new(ConversationId::generate(), Workflow::default(), vec![]);
            Orchestrator::new(
                Arc::new(MockAgentService::default()),
                env(),
                conversation,
                Local::now(),
            )
    ...
    async fn test_render_system_prompt_with_custom_agent() {
        ...
        let context = orch
            .set_system_prompt(
                orch.conversation.context.clone().unwrap_or_default(),
                &custom_agent,
                &Default::default(),
            )
            .await
    ...
    async fn test_render_system_prompt_with_custom_agent_template() {
        ...
        // setup the orch with handlebar
        let mut agent_service = MockAgentService::default();
        agent_service.0.register_template(
            "custom-agent.hbs",
            handlebars::Template::compile(custom_agent_content.into()).unwrap(),
        );
        orch.services = Arc::new(agent_service);
