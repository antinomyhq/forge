...
/// Transformer that normalizes output_format schema to meet Anthropic API requirements.
///
/// Anthropic requires that all object types in JSON schemas must explicitly set
/// `additionalProperties: false`. This transformer recursively processes the schema
...
mod tests {
    ...
    fn test_normalize_output_schema_adds_additional_properties() {
        ...
        if let Some(OutputFormat::JsonSchema { schema }) = actual.output_format {
            ...
            if let Some(nested_schema) = schema_json.get("properties")
            ...
            } else if let Some(defs) = schema_json.get("$defs").or_else(|| schema_json.get("definitions")) {
