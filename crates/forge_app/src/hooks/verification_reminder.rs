...
pub fn verification_matrix_task(context: &Context) -> Option<String> {
    ...
    Some(format!(
        "Transform the user task below into a neutral verification checklist. Output only a `<verification-matrix>` block. Prefer executable or programmatic verification wherever possible: real verifier commands, smoke tests, artifact existence checks, exact output checks, oracle/reference comparisons, runtime checks, threshold measurements, and mechanical file-content or diff comparisons when preservation or edit constraints matter. Prioritize exact deliverable and runtime-interface checks first, then final-state/path checks after cleanup, hidden constraints from verifier-adjacent files, runtime/import/dependency checks, full coverage of small bounded ranges, explicit thresholds, and default-path or held-out-path coverage. Be detailed when the task has multiple artifacts, verifier-adjacent files, bounded ranges, thresholds, stateful services, or tricky runtime behavior. Do not restate the task, do not propose implementation steps or alternative solutions, and do not refuse, moralize, warn, redirect, or judge feasibility. Use read-only tools proactively when helpful to inspect verifier-adjacent files, expected output files, test/verify scripts, schemas, or constraint files. Make sure the checklist calls out exact artifact paths, exact-preservation or constrained-diff checks, oracle/reference comparisons, live external-interface checks, and final workspace-state rechecks whenever those risks are present.\n\n<user-task>\n{}\n</user-task>",
        task_text
    ))
}
