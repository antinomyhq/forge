...
pub fn replace_commands_in_content(
    ...
    for result in results.iter().rev() {
        let replacement = if result.exit_code == 0 {
            if result.stdout.trim().is_empty() {
                "🔧 **Shell Output:** (no output)".to_string()
            } else {
                // Now stdout contains XML format
                result.stdout.clone()
            }
        } else {
            // For errors, also use the XML format
            if result.stdout.trim().is_empty() {
                "🔧 **Shell Error:** (no error message)".to_string()
            } else {
                result.stdout.clone()
            }
        };
...
impl ConcreteInlineShellExecutor {
    pub fn new(
        ...
        Self { command_infra, environment }
    }
}
...
mod tests {
  ...
  <stderr total_lines="1"><![CDATA[Execution error: Error: ]]></stderr>
    ...
    fn test_replace_commands_in_content_no_output() {
        ...
        let actual = replace_commands_in_content(content, &results);
        let expected_contains = "🔧 **Shell Output:** (no output)";

        assert!(actual.contains(expected_contains));
    }
}
