...
use crate::{AuthConfigService, AuthService, Error};

pub struct Authenticator<S> {
    service: Arc<S>,
}

impl<S: AuthConfigService + AuthService> Authenticator<S> {
    ...
    pub async fn logout(&self) -> anyhow::Result<()> {
        if let Some(mut config) = self.service.get_auth_config().await {
            config.key_info.take();
            self.service.set_auth_config(&config).await?;
        }
        Ok(())
    }
    async fn login_inner(&self, init_auth: &InitAuth) -> anyhow::Result<()> {
        let mut config = self.service.get_auth_config().await.unwrap_or_default();
