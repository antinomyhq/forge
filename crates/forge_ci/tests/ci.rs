...
fn generate() {
    ...
    //         )
    //         .add_step(
    //             Step::run("echo \"create_release_id=${{
    // steps.create_release.outputs.id }}\" >> $GITHUB_OUTPUT && echo
    // \"create_release_name=${{ steps.create_release.outputs.tag_name }}\" >>
    // $GITHUB_OUTPUT")                 .id("set_output"),
    //         )
    //         .outputs(indexmap! {
    ...
    //         .cond(Expression::new("(startsWith(github.event.head_commit.message,
    // 'feat') || startsWith(github.event.head_commit.message, 'fix')) &&
    ...
    //         .env(("GITHUB_TOKEN", "${{ secrets.GITHUB_TOKEN }}"))
    //         .env(("APP_VERSION", "${{
    // needs.draft_release.outputs.create_release_name }}"))         .add_step(
    //             Step::uses("test-room-7", "action-publish-release-drafts", "v0")
    //                 .env(("GITHUB_TOKEN", "${{ secrets.GITHUB_TOKEN }}"))
    //                 .add_with(("github-token", "${{ secrets.GITHUB_TOKEN }}"))
    //                 .add_with(("tag-name", "${{
    // needs.draft_release.outputs.create_release_name }}")),         );
    // workflow = workflow.add_job("semantic_release",
    // semantic_release_job.clone());
