use forge_domain::{Error, ToolCallFull, ToolCallId, ToolCallPart, ToolName};
...
fn test_fragmented_tool_call_recovery() {
    ...
    let parts = [
        ...
        ToolCallPart { call_id: None, name: None, arguments_part: "s/c".to_string() },
        ToolCallPart {
            call_id: None,
            name: None,
    ...
    assert!(
        result.is_ok(),
        "Failed to parse fragmented JSON: {:?}",
        result.err()
    );
    ...
    if let serde_json::Value::Object(map) = &tool_calls[0].arguments {
        assert_eq!(
            map["path"].as_str().unwrap(),
            "/Users/amit/code-forge/crates/forge_ci/tests/ci.rs"
        );
...
fn test_error_handling_for_invalid_json() {
    ...
    let parts = [ToolCallPart {
        call_id: Some(ToolCallId::new("call_1")),
        name: Some(ToolName::new("tool_forge_fs_create")),
        arguments_part: "{\"path\": \"test.txt\",".to_string(), /* Invalid JSON (missing closing
                                                                 * brace) */
    }];
