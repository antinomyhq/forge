...
use forge_snapshot::{FileSnapshotService, SnapshotInfo};
use forge_snapshot::service::DefaultSnapshotService;

#[tokio::test]
async fn test_complete_snapshot_workflow() -> Result<(), Box<dyn std::error::Error>> {
    // Setup test environment
    let temp_dir = tempdir()?;
    let test_file = temp_dir.path().join("test.rs");
    let snapshot_dir = temp_dir.path().join("snapshots");

    // Initialize service using public factory method
    let service = DefaultSnapshotService::create(snapshot_dir, 10, 30);
...
#[tokio::test]
async fn test_snapshot_retention_policy() -> Result<(), Box<dyn std::error::Error>> {
    let temp_dir = tempdir()?;
    let test_file = temp_dir.path().join("test.rs");
    let snapshot_dir = temp_dir.path().join("snapshots");

    // Initialize service using public factory method
    let service = DefaultSnapshotService::create(snapshot_dir, 3, 30);
