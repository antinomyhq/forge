...
use crate::{hash_path, get_current_timestamp, format_timestamp};
...
impl DefaultSnapshotService {
    // Private constructor
    fn new(base_dir: PathBuf, max_snapshots: usize, retention_days: u32) -> Self {
        Self {
            base_dir,
            max_snapshots,
            retention_days,
    ...
    // Public factory method that can only be called from forge_main
    pub fn create(base_dir: PathBuf, max_snapshots: usize, retention_days: u32) -> Self {
        Self::new(base_dir, max_snapshots, retention_days)
    }
