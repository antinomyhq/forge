...
use crate::snapshot::{hash_path, get_current_timestamp};
...
impl DefaultSnapshotService {
    ...
    pub async fn generate_diff(&self, file_path: &Path, timestamp: u64) -> Result<String> {
...
impl FileSnapshotService for DefaultSnapshotService {
    ...
    async fn restore_by_index(&self, file_path: &Path, index: usize) -> Result<()> {
        ...
        if index >= snapshots.len() {
            anyhow::bail!("Snapshot index {} out of range", index);
        }
