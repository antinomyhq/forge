...
fn execute_zsh_script_with_streaming(script_content: &str, script_name: &str) -> Result<()> {
    ...
    // On Windows Git Bash, ensure we inherit environment variables for proper execution
    ...
    // On Windows, explicitly pass key environment variables that Git Bash might need
    #[cfg(windows)]
    {
        // Preserve PATH, HOME, and other critical variables for Git Bash/MinTTY
        for key in &["PATH", "HOME", "USERPROFILE", "HOMEDRIVE", "HOMEPATH", "ZDOTDIR"] {
