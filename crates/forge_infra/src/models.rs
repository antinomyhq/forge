...
impl TryFrom<(&forge_domain::Conversation, String)> for NewConversationRecord {
    ...
    fn try_from((conversation, workspace_id): (&forge_domain::Conversation, String)) -> Result<Self, Self::Error> {
...
mod tests {
    use super::*;
    use forge_domain::{Agent, AgentId, Event, Workflow, ConversationId};
    ...
    fn test_try_from_domain_conversation() {
        ...
        let actual = NewConversationRecord::try_from((&fixture, "test_workspace".to_string())).unwrap();
        ...
        let deserialized: forge_domain::Conversation = serde_json::from_str(&actual.context).unwrap();
        assert_eq!(deserialized.id, fixture.id);
    }
    #[test]
    fn test_try_from_domain_conversation_upsert() {
        let fixture = create_test_conversation();
        let actual = UpsertConversationRecord::try_from((&fixture, "test_workspace".to_string())).unwrap();
    ...
    fn test_try_from_conversation_record() {
        let fixture = create_test_conversation();
        let record = NewConversationRecord::try_from((&fixture, "test_workspace".to_string())).unwrap();
    ...
    fn create_test_conversation() -> forge_domain::Conversation {
        ...
        event_data.insert("content".to_string(), Value::String("Hello, world!".to_string()));
    ...
    fn create_conversation_with_long_user_task() -> forge_domain::Conversation {
        ...
        let mut event_data = HashMap::new();
        event_data.insert("content".to_string(), Value::String(long_content.to_string()));
