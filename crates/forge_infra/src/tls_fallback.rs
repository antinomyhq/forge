use crate::tls_retry::{TlsRetryHandler, TlsRetryConfig, diagnostics};
...
impl TlsClientBuilder {
    ...
    pub async fn build_with_fallback(&self) -> Result<Client> {
        if !self.config.tls_fallback_enabled {
            ...
            return self.build_client(None).context("Failed to build HTTP client");
        }
        ...
        Err(last_error.unwrap_or_else(|| anyhow::anyhow!("Failed to build client with any TLS version")))
    }
    ...
    fn configure_rustls_version(&self, mut client: ClientBuilder, version: TlsVersion) -> Result<ClientBuilder> {
        // Note: Direct TLS version configuration for rustls requires access to the underlying
        // rustls configuration. Since reqwest doesn't expose this directly, we'll need to use
        // a custom approach or accept the default rustls configuration which supports TLS 1.2 and 1.3
        match version {
            TlsVersion::Tls10 | TlsVersion::Tls11 => {
                // rustls doesn't support TLS 1.0 or 1.1
                return Err(anyhow::anyhow!("rustls does not support TLS versions below 1.2"));
            }
    ...
    fn configure_native_tls_version(&self, mut client: ClientBuilder, version: TlsVersion) -> Result<ClientBuilder> {
