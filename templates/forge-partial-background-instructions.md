...
- When the task asks you to implement a public function/API that external code will call, design for robustness:
  ...
  2. Follow the target language's established conventions for similar functions. Use documentation tools (fetch, context7, deepwiki) to look up how existing libraries in that language implement comparable APIs before finalizing your signature.
  3. Accept multiple reasonable input formats where the language convention supports it (e.g., bounds as separate arguments OR a single vector). Validate inputs flexibly rather than rigidly rejecting uncommon-but-valid calling patterns.
  4. Test your function with at least two different valid calling conventions (e.g., all positional, all named, mixed) to verify it works for external callers, not just your own test suite.
