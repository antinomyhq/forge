...
- Editing fallback ladder: prefer patch/multi_patch first. If exact-match editing fails repeatedly on long lines, use a guarded shell fallback only for minor token-level replacements (word-boundary matching + expected replacement count). Immediately rerun the diff gate and rollback if any unexpected change appears.
