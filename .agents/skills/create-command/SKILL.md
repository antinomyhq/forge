---
name: create-command
description: Create new commands for the code-forge application. Commands are stored as .md files in the <cwd>/.agents/commands directory with YAML frontmatter (name, description) and markdown body containing command steps. Use when users need to add new commands, modify existing commands, or understand the command file structure. Supports special command tags like <lint> and <test> for automated workflows.
---
...
**CRITICAL**: All command files must be created in the `<cwd>/.agents/commands` directory, where `<cwd>` is the current working directory of your code-forge project.

- **Directory**: `<cwd>/.agents/commands`
- **File format**: `{command-name}.md`
- **Example**: If your project is at `/home/user/my-project`, commands go in `/home/user/my-project/.agents/commands/`
...
Create the file in the `<cwd>/.agents/commands` directory with the format: `{command-name}.md`

**IMPORTANT**: The file MUST be in `<cwd>/.agents/commands` where `<cwd>` is your current working directory. Commands placed anywhere else will not be discovered by forge.
...
- [ ] File is in the `<cwd>/.agents/commands` directory (CRITICAL)
...
- **Directory**: `<cwd>/.agents/commands` (where `<cwd>` is current working directory)
...
1. **Verify the file location**: Ensure the file is in `<cwd>/.agents/commands` directory (CRITICAL - commands anywhere else will not be found)
...
- **File location**: File MUST be in `<cwd>/.agents/commands` directory (this is the most common issue)
