...
use crate::forge_api;
use crate::models::{Message, TestCase};
...
pub async fn token_reduction_test(
    test_case: &TestCase,
    compacted_history: &[Message],
) -> Result<()> {
    println!(
        "\n--- Running Token Reduction Test for '{}' ---",
        test_case.id
    );
    ...
    if compacted_tokens < original_tokens {
        let reduction_percent = (1.0 - (compacted_tokens as f64 / original_tokens as f64)) * 100.0;
        println!("✅ PASS: Token count reduced by {reduction_percent:.2}%.");
    } else {
...
pub async fn information_retrieval_test(
    test_case: &TestCase,
    compacted_history: &[Message],
) -> Result<()> {
    println!(
        "\n--- Running Information Retrieval Test for '{}' ---",
        test_case.id
    );
    ...
    let answer =
        forge_api::answer_question(compacted_history, &test_case.retrieval_test_question).await;
    ...
    if answer
        .to_lowercase()
        .contains(&test_case.expected_answer_keyword.to_lowercase())
    {
